...
def EMS_recovery(
    ...
    R0 = rotations()
    R0.RotM = np.array(
        [-EigVec[:, 0], -EigVec[:, 2], np.cross(EigVec[:, 0], EigVec[:, 2])]
    ).T
    euler0 = R0.euler
    ...
    x0 = np.array(
        [1.0, 1.0, s0[0], s0[1], s0[2], euler0[0], euler0[1], euler0[2], 0, 0, 0]
    )
    ...
    for iterEM in range(MaxIterationEM):
        ...
        optfunc = least_squares(
            CostFunc,
            x,
            bounds=(lb, ub),
            max_nfev=MaxOptiIterations,
            args=(point, p, sigma2),
        )
        x_n = optfunc.x
        ...
        if (cost_n < ToleranceEM and iterEM > 0) or (
            relative_cost < RelativeToleranceEM
            and num_switch >= MaxiSwitch
            and iterEM > 4
        ):
            x = x_n
            break
            ...
            if x[1] <= 1:
                ...
                x_rot[idx, :] = np.array(
                    [
                        x[0],
                        2 - x[1],
                        ((np.sqrt(2) / 2 - 1) * x[1] + 2 - np.sqrt(2) / 2)
                        * min(x[2], x[3]),
                        ((np.sqrt(2) / 2 - 1) * x[1] + 2 - np.sqrt(2) / 2)
                        * min(x[2], x[3]),
                        x[4],
            ...
            if x[0] <= 1:
                ...
                x_rot[idx, :] = np.array(
                    [
                        x[1],
                        2 - x[0],
                        ((np.sqrt(2) / 2 - 1) * x[0] + 2 - np.sqrt(2) / 2)
                        * min(x[3], x[4]),
                        ((np.sqrt(2) / 2 - 1) * x[0] + 2 - np.sqrt(2) / 2)
                        * min(x[3], x[4]),
                        x[2],
...
def CostFunc(x, point, p, sigma2):
    if sigma2 > 1e-10:
        ...
        value = (
            np.abs((p * Distance(point, x) ** 2 + 2 * sigma2 * np.log(SurfaceArea(x))))
            ** 0.5
        )
    return value
...
def SurfaceArea(x):
    ...
    a20 = 4 * (
        x[2] * (x[3] ** 2 + x[4] ** 2) ** 0.5 + x[3] * (x[2] ** 2 + x[4] ** 2) ** 0.5
    )
    a = (x[2] ** 2 + x[3] ** 2) ** 0.5
    b = (x[3] ** 2 + x[4] ** 2) ** 0.5
    c = (x[2] ** 2 + x[4] ** 2) ** 0.5
    ...
    area = (
        np.array([[1 - x[0] / 2, x[0] / 2]])
        @ np.array([[a00, a02], [a20, a22]])
        @ np.array([[1 - x[1] / 2], [x[1] / 2]])
    )
    return area[0, 0]
